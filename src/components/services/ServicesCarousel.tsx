...
export default function ServicesCarousel({
  ...
  const currentItem = serviceList[serviceIndex];
   

  const onClose = () => setSelectedServiceIndex(null);

  const triggerPostChange = (direction: 'up' | 'down') => {
    if (animating) return;

    const newIndex = direction === 'up' ? serviceIndex + 1 : serviceIndex - 1;
    if (newIndex < 0 || newIndex >= serviceList.length) {
      onClose();
      return;
    }
  ...
  useEffect(() => {
    ...
    const handlePopState = () => setSelectedServiceIndex(null);
    window.addEventListener('popstate', handlePopState);
    return () => window.removeEventListener('popstate', handlePopState);
  }, []);

  if (!currentItem || !currentItem.image) return null;
      ...
      >
        <div className="  w-full h-full relative flex items-center justify-center">
          ...
          />
				...
				<div className="p-4 absolute bottom-0 right-0 bg-black/80 w-full h-auto flex flex-col justify-center items-center">
					<h3 className="text-lg lg:text-xl text-white   px-4 py-2 font-bold text-center">{currentItem.title}</h3>
					<p className="text-center text-gray-300   px-4 py-2 font-semibold">{currentItem.price}</p>
				</div>
      </div>
    </div>
  );
