...
export default function AdminNewServiceForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title || !price) {
      setMessageBox({
        message: 'Please fill out both title and price.',
        type: 'error',
      });
      return;
    }
    ...
    try {
      ...
      if (data.status) {
        ...
        setPrice('');
        setMessageBox({
          message: 'Service upload successful!',
          type: 'success',
        });
      } else {
        setMessageBox({
  ...
  return (
    <div className="max-w-3xl mx-auto mt-10 p-4 bg-white shadow-md rounded-lg border border-gray-100">
      <h1 className="text-xl font-semibold mb-4 text-center">Add New Services</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <input
          ...
          placeholder="Enter price (e.g. Rs. 50 - Rs. 100)"
          className="w-full p-2 border border-gray-300 rounded"
