...
const AdminServicesList = ({ serviceList, onDelete, selectService }: Props) => {
  ...
  const handleDelete = async (id: number) => {
    try 
      ...
      setDeleting(true);
			...
			const url = new URL(`${process.env.NEXT_PUBLIC_SITE_URL}/api/services/${id}`);
      ...
      <table className="min-w-full border border-gray-200 whitespace-nowrap">
        ...
        <tbody className="text-md">
          {serviceList.map((service, index) => (
            <tr key={service.id} className="relative">
              <td className="p-2 border">{index + 1}</td>
              <td className="p-2 border">
								<div className="relative w-[40px] h-[40px] rounded shadow-[5px_5px_10px_-3px_rgba(0,0,0,0.6)] hover:opacity-50  overflow-hidden cursor-pointer" onClick={() => selectService(index)}>
									<Image
										src={service.image}
              ...
              <td className="p-2 border  ">{service.created_at}</td>
              <td className="p-2 border  ">
								<div className="relative    inline-block  ">
									...
									{openDropdownId === service.id && (
										<div
											...
											${serviceList.length ==index+1 ? 'bottom-full -translate-y-2' : 'top-full translate-y-2'}		 
