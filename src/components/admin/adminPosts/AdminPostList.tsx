...
const AdminPostList = ({ postList, onDelete, selectPost }: Props) => {
    ...
    try 
      ...
      setDeleting(true);
			if (id === null) return; 
			 ...
			 const url = new URL(`${process.env.NEXT_PUBLIC_SITE_URL}/api/posts/${id}`);
          ...
          {postList.map((post, index) => (
            <tr key={post.id} className="relative">
              <td className="p-2 border">{index + 1}</td>
						 ...
						 <td className="p-2 border flex flex-wrap">
							
							{post.attachment.map((file, fileIndex) => (
								
								<div key={`${post.id}-${file.fileName}-${fileIndex}`}
								className="relative w-[40px] h-[40px] rounded shadow-[5px_5px_10px_-3px_rgba(0,0,0,0.6)] hover:opacity-50 m-1 overflow-hidden cursor-pointer" 
								...
								<div className="relative     inline-block  ">
									...
									{openDropdownId === post.id && (
										<div
											...
											${postList.length ==index+1 ? 'bottom-full -translate-y-2' : 'top-full translate-y-2'}		 
